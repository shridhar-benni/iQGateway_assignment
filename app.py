...
uploaded_file = st.file_uploader("Drag and drop a cell image to determine if it is infected or not", type=["png", "jpg", "jpeg"])
...
if uploaded_file is not None:
    ...
    st.image(test_img, caption="Input image")
    test_ia = test_img.reshape((1, 150, 150, 3))
    
    #load model 
    model = load_model('best_model.h5')
    if model.predict(test_ia)[0][0] > 0.5:
        st.markdown("<p style='color: red; font-weight: bold;'>Infected!</p>", unsafe_allow_html=True)
    else:
        st.markdown("<p style='color: green; font-weight: bold;'>Uninfected!</p>", unsafe_allow_html=True)
    ...
    # Apply a color mapping scheme to convert the heat map to RGB
    heatmap_rgb = plt.get_cmap('jet')(heatmap_plt)[:, :, :3]  # Keep only RGB channels
    ...
    st.image(blended_image, caption="Image over laying both input image and region of influence for classification")
